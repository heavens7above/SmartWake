...
def _patch_dashboards_db(monkeypatch):
    import sys
    # testclient initialized `main` which imported `src.modules.dashboards`.
    # That import is what we need to monkeypatch!
    if "src.modules.dashboards" in sys.modules:
        dashboards = sys.modules["src.modules.dashboards"]
    else:
...
def test_get_dashboard_success(client, api_key, monkeypatch, _patch_dashboards_db):
    ...
    log_rows = [{"timestamp": "2026-04-01T03:00:00Z", "sleep_prob": 0.9, "accel_magnitude": 0.01, "charging": True}]
    ...
    response = client.get("/dashboard?device_id=pytest_sim_device", headers={"X-API-Key": api_key})

    assert response.status_code == 200
    data = response.json()
    assert data["device_id"] == "pytest_sim_device"
    assert data["recent_session"] == session_row
    assert data["logs"] == log_rows
