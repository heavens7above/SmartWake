...
- **`sleep.py`**: The ingestion `/log` API endpoint. Scikit-learn singletons evaluating dictionaries against `sleep_model.pkl`. The 9-dimension Zero-Crossing array math mechanics, and the consecutive Onset-Tracking state machine all neatly tied natively in one wrapper.
...
- **`logger.py`**: A non-blocking thread utilizing `schedule.every(5).minutes`. Polls native Android sensors sequentially (Accelerometer, Battery broadcast receivers, active Notification tray count). POSTs the resultant payload array blindly to our `/log` socket over HTTPs.
