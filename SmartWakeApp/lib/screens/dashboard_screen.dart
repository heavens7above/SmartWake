...
String _formatDateTimeLabel(String? value,
    {int fallbackLength = 19, int parsedLength = 16}) {
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 30), (_) => _load(showLoading: false));
  }
                              ...
                              children: [
                                const Text('SLEEP HISTORY',
                                    style: TextStyle(
                                        ...
                                        letterSpacing: 2)),
                                const Text('Last session & telemetry logs',
                                    style: TextStyle(
...
class _SessionCard extends StatelessWidget {
  ...
  const _SessionCard(
      {required this.session, required this.onRate, this.pending});
        ...
        children: [
          ...
          if (session.onsetTime != null)
            _Row(
                icon: Icons.bedtime,
                label: 'Sleep onset',
                value: _formatDateTimeLabel(session.onsetTime,
                    fallbackLength: 16, parsedLength: 16)),
          if (session.alarmTime != null)
            _Row(
                icon: Icons.alarm,
                label: 'Optimal alarm',
                value: _formatDateTimeLabel(session.alarmTime,
                    fallbackLength: 16, parsedLength: 16)),
          if (session.wakeDeadline != null)
            _Row(
                icon: Icons.flag_outlined,
                label: 'Deadline',
                value: _formatDateTimeLabel(session.wakeDeadline,
                    fallbackLength: 16, parsedLength: 16)),
          const Divider(color: AppTheme.border, height: 24),
          const Text('RATE YOUR SLEEP',
              style: TextStyle(
                  color: AppTheme.textSecond,
                  fontSize: 11,
                  letterSpacing: 1.5)),
